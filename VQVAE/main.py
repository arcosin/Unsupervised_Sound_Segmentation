...
default_hyperparams = {
    'custom': {'lr': 2e-4, 'k': 16384, 'hidden': 128},
...
def main(args):
    ...
    """dataset_names = "./meta/esc50.csv"
    image_path = "./spectrograms/"
    X = []
    y = []
    i = 0
    tensor_transform = transforms.ToTensor()
    with open(dataset_names) as csvfile:
        reader = csv.reader(csvfile, delimiter=',')
        for row in reader:
            if i == 0:
                i += 1
                continue
            file_name = image_path + row[0] + ".png"
            file_class = int(row[2])
            try:
                X.append(tensor_transform(Image.open(file_name)))
    ...
    train_labels = F.one_hot(torch.tensor(y_train))
    test_labels = F.one_hot(torch.tensor(y_test))
    DEVICE = 'cuda:0' if torch.cuda.is_available() else 'cpu'
    train_ds = torch.utils.data.TensorDataset(torch.Tensor(X_train).to(DEVICE), train_labels.to(DEVICE))
    test_ds = torch.utils.data.TensorDataset(torch.Tensor(X_test).to(DEVICE), test_labels.to(DEVICE))
